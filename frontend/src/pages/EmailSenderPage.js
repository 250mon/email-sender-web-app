...
function EmailSenderPage() {
     ...
     */
    const handleReset = () => {
        if (!sending) {  // Prevent reset while sending
    ...
    const showConfirmation = async (emailData, fileFilter, recipients = null) => {
        try {
            ...
            if (!remainingRecipients) {
                ...
                setStatus(remainingRecipients.map(recipient => ({
    ...
    const handleEmailSubmit = async (emailData, fileFilter) => {
        ...
        try {
            ...
            for (let recipient of status) {  // Use status instead of fetching recipients again
                setStatus(prev => prev.map(s => 
        ...
        try {
            ...
            if (skipFutureConfirmations) {
                ...
                for (const recipient of confirmDialog.remainingRecipients) {
                    await handleEmailSubmit(confirmDialog.emailData, confirmDialog.fileFilter, recipient);
                }
                setConfirmDialog(prev => ({ ...prev, open: false }));
            } else {
                // Show confirmation for next recipient
                ...
                >
                    ...
                    <DialogContent sx={{ pt: 1 }}>
                        ...
                        <Box sx={{ mb: 3 }}>
                            ...
                            <Typography>
                                {confirmDialog.emailData?.subject}
                            </Typography>
                        </Box>
                        ...
                        {confirmDialog.remainingRecipients.length > 0 && (
                            ...
                            >
                                {confirmDialog.remainingRecipients.length} more recipient{confirmDialog.remainingRecipients.length > 1 ? 's' : ''} remaining
                            </Typography>
                        )}
                    </DialogContent>
